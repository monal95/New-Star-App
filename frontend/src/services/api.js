// Use the same origin as the current page for API calls
const API_BASE_URL = `${window.location.origin}/api`;

// Helper function to parse order data from API
const parseOrder = (order) => {
  if (!order) return order;
  return {
    ...order,
    shirt:
      typeof order.shirt === "string"
        ? JSON.parse(order.shirt || "{}")
        : order.shirt || {},
    pant:
      typeof order.pant === "string"
        ? JSON.parse(order.pant || "{}")
        : order.pant || {},
  };
};

// Helper function to parse multiple orders
...
export const workAssignmentsAPI = {
  ...
  // Get available items for an order (items not yet fully assigned)
  getAvailableItems: async (orderId) => {
    const response = await fetch(
      `${API_BASE_URL}/workAssignments/available-items/${orderId}`,
    );
